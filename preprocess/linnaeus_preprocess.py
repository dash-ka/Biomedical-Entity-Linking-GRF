...
def main(args):
    ...
    # load corpus
    print("Collecting linnaeus dataset from bigbio hf...")
    corpus = load_dataset("bigbio/linnaeus", "linnaeus_bigbio_kb")
    ...
    for idx, item in tqdm(enumerate(corpus["train"]), total=len(corpus["train"])):
            
        split = "traindev" if idx < 50 else "test"
        pmid = item["document_id"]
        context = "".join(p["text"][0] for p in item["passages"])

        queries = []
        for e in item["entities"]:
            mention = e["text"][0]
            start, end = e["offsets"][0]
            cui = e["normalized"][0]["db_id"]
